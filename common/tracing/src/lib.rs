...
impl Layer {
    ...
    /// Set oth data field names to tracing content.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    fn new_span(&self, attrs: &Attributes, id: &Id, ctx: Context<S>) {
        ...
        let mut oth = String::new();
        attrs.record(&mut SpanVisitor {
            buf: &mut oth,
            futobj_field: None,
            data_field: None,
            oth_fields: self.oth_fields.as_ref(),
        });
    ...
    fn on_record(&self, id: &Id, values: &Record, ctx: Context<S>) {
        ...
        if !fut.is_empty() || !data.is_empty() || !oth.is_empty() {
            let mut buf = String::new();
            write!(&mut buf, "{}", span.name()).unwrap();
...
struct SpanVisitor<'a> {
    ...
    oth_fields: Option<&'a Vec<String>>,
}
